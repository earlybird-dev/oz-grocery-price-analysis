...
def load_woolies_cat_l1_from_big_query():
    ...
    query = 'SELECT * FROM grocery-price-analysis.scraping_data.woolies_cat_l1 WHERE newly_added = 1'
...
def load_woolies_cat_l2_from_big_query():
    ...
    query = 'SELECT * FROM grocery-price-analysis.scraping_data.woolies_cat_l2'
    config_path = path.join(get_repo_path(), 'io_config.yaml')
    config_profile = 'default'
    try:
        data = BigQuery.with_config(ConfigFileLoader(config_path, config_profile)).load(query)
